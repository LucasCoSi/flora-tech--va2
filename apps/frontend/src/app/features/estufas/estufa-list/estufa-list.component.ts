    ...
    <div class="space-y-6">
      <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
        <div>
          ...
          <p class="text-slate-500 mt-1">Gerencie suas unidades hidropônicas</p>
        </div>
              ...
              <div class="p-6 flex-1">
                <div class="flex items-start justify-between mb-4">
                  <div class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium" 
                       [ngClass]="estufa.ativa ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'">
                  ...
                  <div class="flex space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <button (click)="delete(estufa.id)" class="p-1.5 text-slate-400 hover:text-red-600 rounded-md hover:bg-red-50" title="Excluir">
                ...
                <p class="text-xs text-slate-400">
                  Inaugurada em {{ estufa.dataInauguracao | date:'dd/MM/yyyy' }}
                </p>
              </div>
            </div>
          }
...
export class EstufaListComponent implements OnInit {
  ...
  delete(id: number) {
    if (confirm('Tem certeza que deseja excluir esta estufa? Essa ação não pode ser desfeita.')) {
      this.estufaService.delete(id).subscribe({
        next: () => {
          this.toast.success('Estufa excluída com sucesso.');
          this.loadEstufas();
        },
        error: (err: any) => {
          this.toast.error(this.errorService.extractMessage(err));
