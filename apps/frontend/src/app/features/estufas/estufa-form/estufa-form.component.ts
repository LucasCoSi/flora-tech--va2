    ...
    <div class="max-w-2xl mx-auto">
      <div class="flex items-center gap-4 mb-6">
        ...
        <div>
          ...
          <p class="text-slate-500 mt-1">Preencha os dados da sua unidade de cultivo</p>
        </div>
      </div>
        ...
        <form [formGroup]="form" (ngSubmit)="onSubmit()" class="p-6 sm:p-8 space-y-6">
          ...
          <div class="space-y-1">
            ...
            @if (form.get('nome')?.touched && form.get('nome')?.invalid) {
              <p class="text-red-500 text-xs mt-1 ml-1">Nome é obrigatório.</p>
            }
          </div>

          <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div class="space-y-1">
              <label class="block text-sm font-medium text-slate-700 ml-1">Data de Inauguração *</label>
              ...
              @if (form.get('dataInauguracao')?.touched && form.get('dataInauguracao')?.invalid) {
                <p class="text-red-500 text-xs mt-1 ml-1">Data é obrigatória e deve ser válida.</p>
              }
            </div>

            <div class="space-y-1">
              <label class="block text-sm font-medium text-slate-700 ml-1">Área (m²) *</label>
              <div class="relative">
                ...
                <div class="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none">
                  <span class="text-slate-400 sm:text-sm">m²</span>
                </div>
              </div>
              @if (form.get('areaM2')?.touched && form.get('areaM2')?.invalid) {
                <p class="text-red-500 text-xs mt-1 ml-1">Área deve ser um número positivo.</p>
              }
            </div>
          </div>
          ...
          <div class="flex items-center gap-3 p-4 bg-slate-50 rounded-xl border border-slate-100">
            <div class="relative flex items-start">
              ...
              <div class="ml-3 text-sm leading-6">
                ...
                <p class="text-slate-500">Marque se a estufa está atualmente em operação.</p>
              </div>
            </div>
          </div>
...
export class EstufaFormComponent implements OnInit {
  ...
  loadEstufa(id: number) {
    this.estufaService.findById(id).subscribe({
      next: (data) => {
        // Formata data ISO para YYYY-MM-DD do input date
  ...
  onSubmit() {
    ...
    // Converte a data string (YYYY-MM-DD) para ISO completo
    ...
    request.subscribe({
      ...
      error: (err) => {
        ...
        // Exemplo de extração de erros de campo
        const fieldErrors = this.errorService.extractFieldErrors(err);
        if (fieldErrors) {
          // Poderíamos iterar fieldErrors e setar na form, ex: this.form.get('nome').setErrors(...)
